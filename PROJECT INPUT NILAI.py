...
def saveData():
    workbook.save(filename=str(entry_nama.get()) + str(entry_nim.get())+ str(entry_tugas.get())+ str(entry_uts.get())+ str(entry_uas.get())+ str(entry_absen.get())+ ".xlsx")
...
def insertData():
    ...
    DataNIM = sheet['A' + str(sheetnum)]

    sheet['C' + str(sheetnum)] = entry_absen.get()
    DataAbsen = sheet['C' + str(sheetnum)]

    sheet['D' + str(sheetnum)] = entry_tugas.get()
    DataTugas = sheet['D' + str(sheetnum)]

    sheet['E' + str(sheetnum)] = entry_uts.get()
    DataUTS = sheet['E' + str(sheetnum)]

    sheet['F' + str(sheetnum)] = entry_uas.get()
    DataUAS = sheet['F' + str(sheetnum)]
...
button.pack()
button = Button(window, text="Delate", activebackground="red", command=deleteData)
button.pack()
